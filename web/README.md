...
- To **reelle** visningsmoduser (fra Runde 11) — **Bryggelærling** ("veiledet modus, lær mens du brygger") og **Bryggmester** ("full kontroll, alle detaljer tilgjengelig") — samme oppskrift, samme beregningsmotor. Valg skjer i en førstegangsdialog ved første besøk, deretter via en liten status+bryter i venstremenyen (`.modus-knapp`, lagres i `localStorage`); bytte endrer aldri oppskriftsdata. Bryggelærling skjuler brygghuseffektivitet (75 % standard), malt-%-kolonnen, "Bruk prosentfordeling", mål-IBU-feltet og "Skaler oppskrift" — kun enkel kg-flyt. Bryggmester låser opp:
  - **Malt kg ↔ %** (Runde 11, revidert Runde 12C/12D — se `js/app.js::brukMaltProsentfordeling()`): kg er alltid fasit og oppdaterer % live. %-redigering er en egen, eksplisitt retning: mens du skriver i ett eller flere %-felt endres **kun** de feltene + "Prosent-sum" (lest direkte fra synlige input) — kg og andre %-felt er urørt. Hver rad du manuelt redigerer siden forrige kg-endring/vellykkede fordeling regnes som **låst**. Trykker du "Bruk prosentfordeling" beholdes alle låste verdier eksakt; resten (100 % − sum av låste) fordeles proporsjonalt **kun** mellom de urørte radene (én urørt rad får hele resten; flere urørte deler proporsjonalt etter sitt synlige forhold; står alle urørte på 0 % fordeles resten likt). Er *alle* rader manuelt redigert godtas en sum på ~100 % eksakt uten fordeling; ellers endres ingenting og en vennlig melding vises. Total maltvekt bevares alltid (kg = total × %/100, 3 desimaler, avrundingsrest lagt på en urørt rad). Dette er en bevisst, eksplisitt-knapp-basert videreføring av — **ikke** en ordrett kopi av — `ui/malt_panel.py`s enklere "alltid normaliser mot kg-total"-kontrakt.
  - **Skaler oppskrift** (Runde 12, portert fra `ui/recipe_card.py`s "📐 Skaler oppskrift"): eksplisitt knapp i Lagre/eksporter-panelet. Faktor = målvolum ÷ nåværende batchvolum; skalerer malt (kg, 3 desimaler) og humle (gram, 1 desimal) proporsjonalt, oppdaterer batchvolum. Rører aldri gjær, humletid, alfasyre-overstyring, valgt stil, navn/metadata eller egendefinerte ingrediensfelt (til forskjell fra desktop, som i tillegg auto-endrer oppskriftsnavnet — bevisst IKKE portert). Etablerer en ny kg/%-baseline: eventuelle uanvendte %-redigeringer (ikke bekreftet med "Bruk prosentfordeling") nullstilles av skaleringen, selve %-fordelingen forblir uendret.
  - Et mål-IBU→gram-felt per humletilsetning (portert inverse-Tinseth fra `modules/calculations.py::beregn_gram_fra_ibu`, kun via eksplisitt "Beregn gram"-knapp — aldri live, for å unngå en gram/IBU-feedback-loop).
