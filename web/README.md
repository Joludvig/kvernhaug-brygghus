...
- **Hjelp & bryggehåndbok** — 12 hjelpesider under `hjelp/`: `index.html` (kanonisk hub for begrepsforklaringer/FAQ), `bryggedag.html`, `bryggemetoder.html`, `gjaervalg.html`, `trykkgjaering.html`, `sterke-ol.html`, `gjaerhosting.html`, `vannkjemi.html`, `humle.html`, `klaring.html`, `sensorikk.html` og `utstyr-brewzilla.html`. Utvidet fra fire til tolv gjennom innholdsprogrammet Bryggeskole P0–P3B (se [CHANGELOG.md](CHANGELOG.md)).
...
web/
  ...
  verktoy.html          Verktøy -- frittstående bryggeverktøy uten oppskrift/brygg, foreløpig en ABV-kalkulator
                          fra målt OG+FG (issue #77)
  ...
  js/verktoy_page.js      Verktøy-siden (issue #77): live ABV-kalkulator over calc.js sin beregnAbvFraOgFg() --
                          ingen oppskrift-/brygg-state, kun de to tallfeltene på siden selv
  ...
  hjelp/index.html        Hjelp & bryggehåndbok -- kanonisk hub for begrepsforklaringer (#def-*) og FAQ
                          (#faq-*), mål for "Les mer"-lenkene. Andre guidesider omtaler kort og lenker hit
  ...
  hjelp/gjaervalg.html    Gjærvalg -- utgjæring, flokkulering, ester/fenol, alkoholtoleranse (Bryggeskole P3B)
  hjelp/trykkgjaering.html   Trykkgjæring, spunding, closed transfer (Bryggeskole P2B)
  hjelp/sterke-ol.html    Sterke øl / high gravity (Bryggeskole P2C)
  hjelp/gjaerhosting.html    Gjærhøsting og gjenbruk (Bryggeskole P2D)
  hjelp/vannkjemi.html    Avansert vannkjemi (Bryggeskole P2E)
  hjelp/sensorikk.html    Sensorikk og off-flavours (Bryggeskole P2F)
  hjelp/humle.html        Humle i dybden + generell bryggesikkerhet (Bryggeskole P3A)
  hjelp/klaring.html      Klaring og haze -- break, trub, finings, cold crash, chill haze (Bryggeskole P3B)
  hjelp/utstyr-brewzilla.html   Første utstyrsspesifikke guide (og uformell mal for flere)
  en/                    GENERERT engelsk speiling av de 20 sidene over (samme filnavn/struktur, én katalognivå
                          ...
                          (web/en/)" under.
  sitemap.xml             GENERERT -- 38 URL-er (19 sider x NO/EN; utskrift.html er bevisst ekskludert) med
...
**Arkitektur**: `js/i18n.js` er lastet FØRST på hver side (før `chrome.js`) og eksponerer globalt: `t(nøkkel, params)` (enkel `{param}`-substitusjon, ingen pluralregler), `gjeldendeSprak()`, `settSprak(kode)`, `applyI18n(root)`. Statisk HTML dekoreres med `data-i18n="nøkkel"` (setter `textContent`), `data-i18n-html="nøkkel"` (setter `innerHTML` — for tekst med inline-markup som `<strong>`/`<a href="#anker">`) og `data-i18n-placeholder`/`data-i18n-aria-label`/`data-i18n-title`/`data-i18n-alt` (setter tilsvarende attributt) — anvendes automatisk på `DOMContentLoaded`. Dynamiske strenger (statusmeldinger, `confirm()`-dialoger, stilmatch-/veiledningstekst, print-dokumentene) kaller `t()` direkte i JS i stedet for å hardkode norsk. Alle tekster ligger i én flat, dobbel `TEKSTER = { no: {...}, en: {...} }`-ordbok i `i18n.js` — 623 nøkkelpar (Runde 20), verifisert NO/EN-symmetrisk av `scripts/generate_web_i18n_pages.py` ved hver generering.
...
**Meta description**: samme arkitektur som title — én nøkkel per side og språk i `TEKSTER` (`meta.X.beskrivelse`, 20 par NO/EN), referert fra `<meta name="description" content="..." data-i18n-content="nøkkel">` i HTML-en via en ny `data-i18n-content`-attributt (samme mønster som `data-i18n-alt`/`-title`). Norsk kilde har den norske teksten statisk i `content`; generatoren overskriver den til engelsk for `web/en/`, akkurat som for `data-i18n`/`data-i18n-html`.

**Canonical + hreflang**: alle 40 sider har `<link rel="canonical">` (selvrefererende, absolutt HTTPS) og tre `<link rel="alternate" hreflang="...">` (`no`, `en`, `x-default` → alltid norsk). De 20 norske kilde-sidene har disse fire lenkene satt statisk i `<head>` (satt for NO-konteksten); generatoren overskriver dem til riktige EN-URL-er for `web/en/`-speilingen — samme "generator overskriver, eier ikke teksten selv"-prinsipp som resten av transformasjonen. Verifisert gjensidig for alle 20 par (NO→EN, EN→NO, begge x-default→NO) og selvreferanse for alle 40 canonical-lenker.

**sitemap.xml + robots.txt**: `web/sitemap.xml` genereres deterministisk fra samme `PAGES`-liste som resten av generatoren, minus `SITEMAP_EKSKLUDERT` (sider som fortsatt genereres/valideres normalt, men bevisst utelates fra selve sitemap-outputen — pr. i dag kun `utskrift.html`, som har `<meta name="robots" content="noindex, follow">` etter SEO-audit 2026-08) — `<url>`-entries med gjensidige hreflang-alternates (xhtml-namespace), ingen `lastmod`/`priority`/`changefreq` (ville enten vært falsk eller ikke-deterministisk). `web/robots.txt` er en liten, håndskrevet statisk fil (`Allow: /`, pekt til sitemap) — ingen sider/mapper er blokkert.
...
`hjelp/` er egne, statiske sider (samme `style.css` for visuell identitet, ingen delt kjøretid med byggeren) som åpnes i ny fane fra "📖 Hjelp"-lenken i header og fra "? → Les mer"-lenker i hjelpepopoverne. `hjelp/index.html` samler kom-i-gang, begrepsforklaringer (med stabile anker-IDer som `#og`/`#ibu`/`#stilmatching` osv. — disse er "Les mer"-lenkenes mål og må ikke endres uten å oppdatere `HJELP_TEKSTER.lesMer` i `help.js` samtidig), ingrediensstoff og FAQ. De elleve øvrige sidene er egne temaguider: `bryggedag.html` (full bryggedagsguide), `bryggemetoder.html` (metodesammenligning), `gjaervalg.html`, `trykkgjaering.html`, `sterke-ol.html`, `gjaerhosting.html`, `vannkjemi.html`, `humle.html`, `klaring.html`, `sensorikk.html` og `utstyr-brewzilla.html` (utstyrsspesifikke guider).

`hjelp/index.html` er **kanonisk hub** for begrepsforklaringer (`#def-*`) og FAQ (`#faq-*`): de øvrige guidesidene gir en kort kontekstuell omtale og lenker dit i stedet for å gjenta definisjonen. Faglig nyansering er et krav, ikke en stilpreferanse — modeller (f.eks. Tinseth) omtales som modeller, og "kan/ofte/typisk/avhenger av" brukes framfor absolutte fasiter. `hjelp-side-nav` er gruppert: de 12 chipsene ligger i tre kategorier — **KOM I GANG** (Hjelp & FAQ, Bryggedag, Bryggemetoder), **BRYGGMESTER** (Gjærvalg, Trykkgjæring, Sterke øl, Gjærhøsting, Vannkjemi, Humle, Klaring, Sensorikk) og **UTSTYR** (BrewZilla). Grupperingen er ren HTML/CSS uten JS: `.hjelp-nav-gruppe` med `.hjelp-nav-gruppe-tittel` og `.hjelp-nav-lenker` inne i den eksisterende flex-wrap-nav-en. Mobil og tablet bruker responsiv flattening — under 900 px skjules gruppetitlene og chipsene flyter ut som én wrappende rad (`display: contents`), slik at navigasjonen ikke blir unødvendig høy; desktop beholder grupperingen. Samme 12 chips, samme aktive tilstand og samme 44 px touch-target i begge tilstander.
