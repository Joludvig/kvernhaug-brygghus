...
- Stilmatching mot **Kvernhaug Brygghus sitt eget stilbibliotek** (26 stiler): numerisk nærmeste stil, nærliggende alternativer med "hva mangler", og manuelt valg av en spesifikk stil å sjekke oppskriften mot
...
Ingen delt kjøretid mellom Python- og JS-siden — alt under `js/` (unntatt `combobox.js` og `radar.js`, som er egne web-komponenter uten Python-motstykke) er manuelle porter som må oppdateres for hånd hvis kilden i `modules/` endres:
