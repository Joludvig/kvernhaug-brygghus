...
- **HO — felles siste steg for lokale jobber og Agent Bridge:** følg [HO policy — local and GitHub jobs](docs/development/AGENT_WORKFLOW.md#ho-policy--local-and-github-jobs). Vurder materiell operativ endring etter verifisering, oppdater bare gjeldende AUTO/ROLLING via #152-ruteren som eneste avklarte skriver, og rapporter `HO UPDATED: YES/NO` med grunn. STATIC krever eksplisitt autorisasjon. Samme policy avklarer eldre HO-/Vault-prompter; ikke opprett en parallell handover.
...
Produktdokumentasjon (hva appen *kan*, status, dataflyt): [docs/ROADMAP.md](docs/ROADMAP.md) (historisk «Ferdig»-status er fortsatt gyldig; se pekeren øverst i filen). Gjeldende nær-tids prioriteringsrekkefølge er GitHub issue #101 («ROADMAP V2.1 — Stabilization & Prove-It», adoptert 2026-09-07); issue #78 («ROADMAP V2 — Product Phase») forblir gyldig som bredere/nordstjerne-kontekst, ikke som nær-tids fasit. Autoritativ kilde for **gjeldende** implementasjons-/livssyklustilstand er GitHub master pluss aktive Issues/PR-er (nær-tids roadmap-retning: issue #101; bredere kontekst: issue #78) — `docs/PROJECT_STATUS_*.md`, inkl. nyeste [docs/PROJECT_STATUS_AUGUST_2026.md](docs/PROJECT_STATUS_AUGUST_2026.md), er punkt-i-tid-historikk (jf. issue #99), ikke løpende teknisk sannhet. Se også [docs/MASTER_DATA_FLOW.md](docs/MASTER_DATA_FLOW.md).
