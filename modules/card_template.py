...
# ── Kortets iframe-høyde ───────────────────────────────────────────────────
# render_card_html() returnerer en <div> UTEN egen max-height/overflow — den
# vokser fritt med innholdet. Streamlit rendrer den likevel inne i en
# st.components.v1.html(..., height=..., scrolling=False)-iframe (se
# ui/recipe_card.py), og DET er en fast, ekstern pikselhøyde — innhold som
# er høyere enn den blir klippet ved selve iframe-kanten, uavhengig av CSS
# på kortet selv. Denne høyden må derfor beregnes ut fra faktisk innhold
# (særlig tittelens linjebrudd, som en tidligere fast konstant ikke tok
# hensyn til — se beregn_recipe_card_height()).
_CARD_BASE_HEIGHT_PX = 1080          # header+ornamenter+stats+seksjonshoder+gjær+footer, ÉN tittellinje forutsatt
_CARD_TITLE_LINE_HEIGHT_PX = 50      # 2.15em fet uppercase serif, line-height 1.1 + bokstavavstand
_CARD_TITLE_AVG_CHAR_WIDTH_PX = 21   # konservativt anslag for samme tittelstil
_CARD_WIDTH_PX = 560                 # omtrentlig bredde kortet faktisk får i col2 (se ui/recipe_card.py)
_CARD_MALT_ROW_HEIGHT_PX = 30
_CARD_HUMLE_ROW_HEIGHT_PX = 30
_CARD_MALT_ROWS_INCLUDED = 3
_CARD_HUMLE_ROWS_INCLUDED = 3
_CARD_BRYGGER_STIL_EXTRA_PX = 30
_CARD_SUMMARY_LINE_HEIGHT_PX = 26    # 0.91em, line-height 1.58
_CARD_SUMMARY_AVG_CHAR_WIDTH_PX = 7.2
_CARD_SUMMARY_WIDTH_PX = 480         # innenfor smaksprofil-boksens egen padding
_CARD_SUMMARY_LINES_INCLUDED = 2
_CARD_BOTTOM_MARGIN_PX = 30          # trygg bunnmargin -- alltid lagt til, aldri fjernet


def _estimer_linjer(tekst: str, bredde_px: float, snitt_tegnbredde_px: float) -> int:
    """Grovt, men bevisst KONSERVATIVT (heller for mange enn for få linjer)
    anslag på hvor mange linjer en tekst bryter til ved en gitt bredde og
    gjennomsnittlig tegnbredde. Returnerer 0 for tom tekst."""
    tekst = (tekst or "").strip()
    if not tekst:
        return 0
    tegn_per_linje = max(1, int(bredde_px / snitt_tegnbredde_px))
    return max(1, -(-len(tekst) // tegn_per_linje))  # heltalls-opprunding


def beregn_recipe_card_height(
    recipe: dict,
    summary_tekst: str = "",
    card_width_px: float = _CARD_WIDTH_PX,
) -> int:
    """Beregner nødvendig høyde (px) for oppskriftskortets
    st.components.v1.html(..., height=...)-iframe — INGEN fast konstant.

    Tar hensyn til: antall tittellinjer (den faktiske roten til
    avkappingsbugen — en lang tittel som «Kvernhaug Wiesn-Märzen 1872»
    brytes over to linjer og gjør kortet reelt høyere enn ett med en kort,
    én-linjes tittel som «Eldsvenn V1»), antall malt-/humlerader utover det
    som er forutsatt i grunnhøyden, en lengre smaksprofiltekst, og
    bryggerstil-linjen. Legger alltid til en trygg bunnmargin.

    Ren Python, ingen Streamlit — testbar helt isolert fra selve
    rendringen (se tests/test_card_template_height.py)."""
    tittel = recipe.get("name", "") or ""
    n_malt = len(recipe.get("malts") or [])
    n_humle = len(recipe.get("hops") or [])
    brygger_stil = (recipe.get("brygger_stil") or "").strip()

    tittel_linjer = _estimer_linjer(tittel, card_width_px, _CARD_TITLE_AVG_CHAR_WIDTH_PX)
    ekstra_tittel_linjer = max(0, tittel_linjer - 1)

    summary_linjer = _estimer_linjer(summary_tekst, _CARD_SUMMARY_WIDTH_PX, _CARD_SUMMARY_AVG_CHAR_WIDTH_PX)
    ekstra_summary_linjer = max(0, summary_linjer - _CARD_SUMMARY_LINES_INCLUDED)

    hoyde = _CARD_BASE_HEIGHT_PX
    hoyde += ekstra_tittel_linjer * _CARD_TITLE_LINE_HEIGHT_PX
    hoyde += max(0, n_malt - _CARD_MALT_ROWS_INCLUDED) * _CARD_MALT_ROW_HEIGHT_PX
    hoyde += max(0, n_humle - _CARD_HUMLE_ROWS_INCLUDED) * _CARD_HUMLE_ROW_HEIGHT_PX
    hoyde += ekstra_summary_linjer * _CARD_SUMMARY_LINE_HEIGHT_PX
    if brygger_stil:
        hoyde += _CARD_BRYGGER_STIL_EXTRA_PX
    hoyde += _CARD_BOTTOM_MARGIN_PX
    return hoyde
