...
porten kjører for runde 1. `vurder_ready` under kjører likevel identisk
for HVER vellykket runde (ingen spesialkasus for trigger-etiketten); når
PR-en allerede er Ready, er utfallet ganske enkelt et IKKE-alarmerende
no-op (`already_ready=True`) -- ingen mutasjon skjer, og runde 1s
eksisterende oppvåkning via GitHubs `opened`-PR-hendelse + Chief-ready-
markøren fortsetter helt uforstyrret.
...
CLI-bruk (det workflowen gjør):
    jq -n '{issue_number: 44, issue_labels: [...], prs: [...],
            ...
            comments: [...], reviews: [...]}' \
      | python3 .github/scripts/pr_ready_handoff.py
Skriver GITHUB_OUTPUT-linjer (`set_ready`, `already_ready`, `pr_number`,
`head_sha`, `reason`) til stdout og begrunnelsen til stderr.
...
def vurder_ready(*, issue_nummer, issue_labels, prs, branch_navn, signalert_head_sha,
    ...
    IKKE-alarmerende no-op (dekker runde 1 og gjentatte kjøringer for
    samme hode); enhver annen `set_ready=False` er en fail-closed
    AVVISNING (se moduldocstring, akseptansekriterium 3).
    """
    ...
    if not pr.get("isDraft"):
        return False, True, pr_nummer, signalert_head_sha, (
            f"PR #{pr_nummer} er allerede Ready (ikke Draft) -- idempotent no-op "
            "(dekker runde 1, som aldri gikk via Draft, og gjentatte kjøringer for "
            "samme hode)."
        )
