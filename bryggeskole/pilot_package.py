"""
Bryggeskole package-fundamentals pilot -- the fifth Bryggeskole learning
slice (issue #374, V2.2 Goal 3G), built on top of the Course Fact
Registry's trusted, verified-only consumer API
(bryggeskole/course_fact_registry.py, issue #91), mirroring
bryggeskole/pilot_mashing.py, bryggeskole/pilot_fermentation.py,
bryggeskole/pilot_boil_hop.py and bryggeskole/pilot_cool_transfer.py's
architecture and validation rules exactly -- this module is deliberately
its own topic-scoped copy, not a generalization of those modules, since
none of the five pilots hardcodes anything beyond its own default file
paths and each is consumed directly by topic (never through a shared
"pilot engine" import).

Scope, stated plainly (mirrors the governing issue and
docs/development/v22_g3f_package_module_contract.md):
- one bounded pilot topic -- package fundamentals -- consuming two
...
- the module's own critical wording guardrails (issue #374 §3): never
  teach a single universal priming-sugar amount, a single universal keg
  pressure/temperature/time schedule, that force carbonation is
  inherently better than bottle conditioning, that bottling is merely a
  beginner method or kegging the "serious brewer" path, that any oxygen
  exposure after fermentation instantly ruins beer, specific safe/unsafe
  bottle/keg brands, pressure ratings/burst-pressure engineering, or
...
DEFAULT_PILOT_PATH = os.path.join(_HERE, "data", "pilot_package_fundamentals.json")
