"""
Bryggeskole method-context-fundamentals pilot -- the sixth Bryggeskole
learning slice (issue #380, V2.2 Goal 3I), built on top of the Course
Fact Registry's trusted, verified-only consumer API
(bryggeskole/course_fact_registry.py, issue #91), mirroring
bryggeskole/pilot_mashing.py, bryggeskole/pilot_fermentation.py,
bryggeskole/pilot_boil_hop.py, bryggeskole/pilot_cool_transfer.py and
bryggeskole/pilot_package.py's architecture and validation rules exactly
-- this module is deliberately its own topic-scoped copy, not a
generalization of those modules, since none of the six pilots hardcodes
anything beyond its own default file paths and each is consumed
directly by topic (never through a shared "pilot engine" import).

Scope, stated plainly (mirrors the governing issue and
docs/development/v22_g3h_prepare_method_module_contract.md):
- one bounded pilot topic -- method-context fundamentals (BIAB,
  traditional separate-vessel all-grain, all-in-one brewing machines) --
  consuming two reused verified records (FACT-MASH-0001,
...
- a pure, fail-closed content validator plus pure renderer/view-model
  helpers only -- no Streamlit/HTML/web dependency, no persistent
  mastery state, no adaptive scheduling, and no raw-score UX
...
- the module's own critical wording guardrails (issue #380 §3): never
  teach BIAB as "beginner-only", traditional separate-vessel brewing as
  "proper"/inherently superior, all-in-one as inherently easier/better/
  worse, any method as producing inherently better beer, one fixed
  water/dead-space/efficiency number across systems, brand/model
  purchase guidance, a full sparging-technique lesson, or professional
  brewery method depth.
- `method.shared_process` is a module-local mastery concept only -- the
  Registry `concepts` on FACT-MASH-0001/FACT-BOIL-0001 are unchanged,
  per the governing contract's §8 Chief decision.
...
DEFAULT_PILOT_PATH = os.path.join(_HERE, "data", "pilot_method_context_fundamentals.json")
...
def validate_pilot_content(data, registry_path=DEFAULT_REGISTRY_PATH):
    ...
    so a missing claim id and an id that exists but is not `verified`
    both fail identically (§18.3 of BRYGGESKOLE_COURSE_FACT_REGISTRY_V1.md).
