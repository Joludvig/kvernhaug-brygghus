"""
Bryggeskole boil/hop-fundamentals pilot -- the third Bryggeskole learning
slice (issue #366, V2.2 Goal 3C), built on top of the Course Fact
Registry's trusted, verified-only consumer API
(bryggeskole/course_fact_registry.py, issue #91), mirroring
bryggeskole/pilot_mashing.py and bryggeskole/pilot_fermentation.py's
...
none of the three pilots hardcodes anything beyond its own default file
paths and each is consumed directly by topic (never through a shared
"pilot engine" import).

Scope, stated plainly (mirrors the governing issue and
docs/development/v22_g3b_boil_hop_module_contract.md):
- one bounded pilot topic -- boil/hop fundamentals -- consuming only the
  seven verified FACT-BOIL-0001..0004 and FACT-HOP-0001..0003 records;
- six bilingual (NO/EN) learning chunks, plus eight questions/scenarios,
...
- deliberately not an IBU math lesson: no chunk or question asks the
  learner to derive or compute a Tinseth utilization number
  (modules/calculations.py already owns that calculation).
