"""
Bryggeskole mash-fundamentals pilot -- productionized Mesking learning
content (issue #337), the second small end-to-end Bryggeskole learning
slice, built on top of the Course Fact Registry's trusted, verified-only
consumer API (bryggeskole/course_fact_registry.py, issue #91), mirroring
the fermentation-temperature pilot's architecture and validation rules
(bryggeskole/pilot_fermentation.py, issue #95) exactly -- this module is
deliberately its own topic-scoped copy, not a generalization of that
module, since neither pilot hardcodes anything beyond its own default
file paths and both are consumed directly by topic (never through a
shared "pilot engine" import).

Scope, stated plainly (mirrors the governing issue):
- one bounded pilot topic -- mash fundamentals -- consuming only the
  verified FACT-MASH-0001, FACT-MASH-0002 and FACT-MASH-0004 records.
  FACT-MASH-0003 (the iodine test) is deliberately NOT referenced here:
  it was left at status `draft` in the registry pending an independent
  second source, per issue #337's own instruction to weaken/drop a
  claim rather than force its promotion;
- three bilingual (NO/EN) learning chunks, plus three questions/
  scenarios, each declaring source_claims that must resolve to a
  *verified* Course Fact Registry record;
...
DEFAULT_PILOT_PATH = os.path.join(_HERE, "data", "pilot_mashing_fundamentals.json")
