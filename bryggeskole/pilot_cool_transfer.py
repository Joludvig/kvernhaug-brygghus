"""
Bryggeskole cool/transfer-fundamentals pilot -- the fourth Bryggeskole
learning slice (issue #370, V2.2 Goal 3E), built on top of the Course
Fact Registry's trusted, verified-only consumer API
(bryggeskole/course_fact_registry.py, issue #91), mirroring
bryggeskole/pilot_mashing.py, bryggeskole/pilot_fermentation.py and
bryggeskole/pilot_boil_hop.py's architecture and validation rules
exactly -- this module is deliberately its own topic-scoped copy, not a
generalization of those modules, since none of the four pilots
hardcodes anything beyond its own default file paths and each is
consumed directly by topic (never through a shared "pilot engine"
import).

Scope, stated plainly (mirrors the governing issue and
docs/development/v22_g3d_cool_transfer_module_contract.md):
- one bounded pilot topic -- cool/transfer fundamentals -- consuming
  only the seven verified FACT-COOL-0001..0003, FACT-OXY-0001..0003 and
  FACT-TRANSFER-0001 records;
...
- the module's own critical oxygen guardrail (issue #370 §2): never
  teach "always aerate before pitching", "oxygen is always wanted
  before pitching", "oxygen is always bad", or that dry yeast never
  benefits from oxygen under any circumstances -- the qualified,
  timing-and-need-dependent rule is what every chunk/question must
  express.
...
DEFAULT_PILOT_PATH = os.path.join(_HERE, "data", "pilot_cool_transfer_fundamentals.json")
