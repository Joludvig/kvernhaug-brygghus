...
- **Web-versjon V1** (`web/`, HEAD `d974772`) — separat, offentlig, forenklet nettversjon av oppskriftsbyggeren: OG/FG/ABV/IBU/EBC, smakshjul (vanilla SVG), søkbare dropdown-felt (malt/humle/gjær/stil — søkbart på bl.a. produsent/opprinnelse/type) og stilmatching mot Kvernhaug Brygghus sitt eget 26-stils bibliotek (**ikke** hele det offisielle BJCP-heftet — se [../web/README.md](../web/README.md) for presis dekning), lokal lagring i nettleser, print, JSON-eksport/import. To visningsmoduser — **Bryggelærling** (veiledet, med "?"-hjelpetekster på sentrale begreper) og **Bryggmester** (full detalj) — deler samme oppskrift og beregningsmotor. Stilmatchen har en vennlig tre-nivås stilveiledning i stedet for feilmeldinger. Egendefinerte ingredienser (malt/humle/gjær) støttes per oppskrift, inkl. alfa-overstyring på biblioteks-humle. Ingrediens-/stildata regenereres fra desktop sine masterdata via `scripts/generate_web_data.py` — ingen egen, manuelt vedlikeholdt web-database. Bygget og test-/polish-rundet 2026-08-10, committet men ikke deployet ennå.
...
6. **Web-versjon — neste runde**: egen Hjelp/How-to/FAQ-side (med "? → Les mer"-kobling fra de eksisterende hjelpeknappene), generell bryggedagsguide, bryggemetoder, utstyrsspesifikke guider (f.eks. BrewZilla), brukeridentitet (bryggernavn), og egne utskriftsmaler for oppskrift, handleliste, bryggedag og bryggelogg.
