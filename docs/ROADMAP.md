...
*Sist oppdatert: 2026-08-14 (Web Runde 14 — norsk/engelsk UI, godkjent og committet). Se `docs/PROJECT_STATUS_JULI_2026.md` for full status, nøkkeltall og kjent teknisk gjeld.*
...
- Web: norsk/engelsk UI (Runde 14) — vanilla NO/EN-støtte (`js/i18n.js`, ingen npm/build-steg/tredjepartsbibliotek) på tvers av bygger, Mine oppskrifter, Importer, Utskrift, print-dokumentene og first-run modusdialog. Norsk fortsatt default; språkbytte er live (ingen reload, ingen tap av data) og lagres i en egen preferansenøkkel aldri koblet til selve oppskriftsdataen. Stilnavn/smakskategorier/maltgrupper løst som et rent visningslag oppå den eksisterende norske identiteten (`valgtStil` m.fl. uendret i lagring/logikk) — se `web/README.md` "Språk (NO/EN)" for full arkitektur og URL-strategi (klientside, samme URL i V1 — kjent SEO-begrensning, egen fremtidig runde). `hjelp/`-sidenes navigasjon er oversatt; selve brødteksten er bevisst utsatt til Runde 14B, se «Neste» under.
...
6. **Web: Hjelp-innhold på engelsk (Runde 14B)** — Runde 14 leverte NO/EN-arkitekturen og oversatte hele app-UI-et, men `hjelp/`-sidenes faktiske brødtekst (`hjelp/index.html`s glossar/FAQ, `bryggedag.html`, `bryggemetoder.html`, `utstyr-brewzilla.html` — til sammen ~900 linjer) er bevisst utsatt til en egen runde, med et synlig varsel i UI-et i mellomtiden. Naturlig, tydelig bryggeengelsk — ikke maskinoversettelse.
7. **Web — fremtidig SEO-URL-strategi**: Runde 14 valgte klientside språkbytte på samme URL for V1 (se `web/README.md` "Språk (NO/EN)"). Skal engelsk innhold være reelt søkbart, trengs et avgrenset pre-render-steg ved deploy (statiske `/en/*.html`-snapshots generert fra samme `TEKSTER`-ordbok) — egen runde, ikke påbegynt. Del av punkt 8 sitt SEO-grunnarbeid.
8. **Web-versjon — Pre-deploy / lanseringsklar-runde** (før publisering på `KvernhaugBrygghus.no`, ikke påbegynt): vurdere endelig funksjonsomfang for første offentlige versjon, innholdskvalitet i Hjelp/bryggehåndbok, datakvalitet for malt/humle/gjær, stilbibliotek/dekning, visuell sluttpolish på mobil/desktop, utskriftskontroll med ekte oppskrifter, hosting/opplasting, kobling til `KvernhaugBrygghus.no`, og en produksjonstest etter deploy. Kjente delkrav som ikke må forsvinne før lansering:
   - **Norsk + engelsk er must-have** — app-UI ferdig (Runde 14); Hjelp-innhold gjenstår (punkt 6), egen crawlbar engelsk URL gjenstår (punkt 7).
