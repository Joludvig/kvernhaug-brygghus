...
8. **Web-versjon — Pre-deploy / lanseringsklar-runde** (før publisering på `KvernhaugBrygghus.no`). **Runde 16 (gjennomført, godkjent 2026-08-14):** full pre-deploy-kontroll — 0 blokkerende funn, én liten innholdsfeil rettet (utdatert JSON-referanse i to norske Help-tekster). Kjente delkrav som ikke må forsvinne før lansering:
   ...
   - ~~Kontakt-e-post — mangler et sted brukere kan nå Kvernhaug Brygghus.~~ — **Ferdig (Runde 17).** `post@kvernhaugbrygghus.no` (bekreftet, opprettet hos Domeneshop) — mailto-lenke i footer på alle 18 sider + egen `personvern.html`/`en/personvern.html`.
   - ~~Personvern-/cookieside eller -tekst — mangler, avhenger av faktisk funksjonalitet ved lansering.~~ — **Ferdig (Runde 17).** Egen «Kontakt og personvern»-side (NO/EN), lenket fra footer på alle sider — kort, ærlig forklaring av lokal lagring/`.kbhrecipe`, ingen egen analytics/reklame/sporing. Ikke cookie-banner (ingen tracking-cookies finnes). Produksjons-smoke-test av URL-strukturen (`/`, `/en/`, `/hjelp/`, `/en/hjelp/` osv.) og faktisk Search Console-innsending står fortsatt igjen — kan først gjøres etter selve deploy.
