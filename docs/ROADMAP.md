...
> **Superseded som prioriteringskilde (2026-09-06, oppdatert 2026-09-07):** GitHub issue #78 («ROADMAP V2 — Product Phase») erklærte denne filen «materially stale compared with current master» som planleggingsgrunnlag og etablerte prioritert rekkefølge for neste produktfase der i stedet, fra master-baseline `57cde82871e65f1a8414a5e474c4003cb5d2e308`. Issue #78 forblir gyldig som bredere/nordstjerne-kontekst (produktretning på lengre sikt), men **gjeldende nær-tids prioriteringsrekkefølge styres nå av GitHub issue #101 («ROADMAP V2.1 — Stabilization & Prove-It»)**, adoptert 2026-09-07 fra master-baseline `51c47272a0a8aaa134a5af373bff291eb9d69757`. Denne filen rørt ikke — «Ferdig»-listen under er fortsatt en gyldig historisk oversikt over hva som faktisk er bygget, den er bare ikke lenger fasit for *hva som prioriteres videre*. Se issue #101 for gjeldende nær-tids rekkefølge, og issue #78 for den bredere nordstjerne-konteksten.
...
7. ~~**Web — fremtidig SEO-URL-strategi**~~ — **Ferdig (Runde 15A–15B.4, 2026-08-14).** Egen, ekte crawlbar engelsk URL-struktur under `/en/**` (9 sider, speilet 1:1), generert deterministisk av `scripts/generate_web_i18n_pages.py` fra norsk kilde-HTML + `TEKSTER` i `i18n.js` — ingen manuell dobbel vedlikeholdsbyrde. Fullt teknisk SEO-grunnlag på plass: unik `<title>`/meta description per side og språk, selvrefererende canonical, gjensidig hreflang no/en/x-default, `web/sitemap.xml` (18 URL-er) og `web/robots.txt`. Se `web/README.md` "Engelsk pre-render (web/en/)" og "URL-kontrakt".
8. ~~**Web-versjon — Pre-deploy / lanseringsklar-runde**~~ — **Ferdig (Runde 16–18, deployet 2026-08-14).** Full pre-deploy-kontroll (Runde 16, 0 blokkerende funn), kontakt/personvern (Runde 17), og selve produksjonsdeployen (Runde 18): `https://kvernhaugbrygghus.no` er nå live hos Domeneshop, `origin/master` = commit `56c924e`. Full produksjons-smoke rett etter deploy var grønn (alle 18 sider NO/EN, raw SEO, sitemap.xml, robots.txt, HTTP→HTTPS-redirect, TLS). Samme dag: en produksjonsbug ble oppdaget (EN-builderens "Beer name"-felt viste den norske interne sentinelen "Uten navn" i stedet for en lokalisert placeholder) og rettet i commit `56c924e` — se `web/CHANGELOG.md` "Runde 18" for full rotårsak og fiks. Et første FTP-opplastingsforsøk av hotfixen traff feil lokal kildemappe (en eldre deploy-/scratch-kopi, ikke selve repoets `web/js/`) og resulterte i en midlertidig deploy mismatch — IKKE en cache- eller kodefeil; løst ved ny opplasting fra riktig kildemappe, verifisert byte-identisk mot `HEAD` og bekreftet grønn i produksjon.
   - **Norsk + engelsk er must-have** — app-UI og hjelp/bryggehåndbok begge ferdig oversatt (Runde 14 + 14B); egen crawlbar engelsk URL OG teknisk SEO-grunnarbeid er nå ferdig (punkt 7); Bryggmester-innholdshullene i punkt 6 bør også vurderes i en fremtidig Help-innholdsrunde (ikke en lanseringsblocker).
   - ~~Kontakt-e-post — mangler et sted brukere kan nå Kvernhaug Brygghus.~~ — **Ferdig (Runde 17).** `post@kvernhaugbrygghus.no` (bekreftet, opprettet hos Domeneshop) — mailto-lenke i footer på alle 18 sider + egen `personvern.html`/`en/personvern.html`, verifisert live i produksjon.
   - ~~Personvern-/cookieside eller -tekst — mangler, avhenger av faktisk funksjonalitet ved lansering.~~ — **Ferdig (Runde 17).** Egen «Kontakt og personvern»-side (NO/EN), lenket fra footer på alle sider — kort, ærlig forklaring av lokal lagring/`.kbhrecipe`, ingen egen analytics/reklame/sporing. Ikke cookie-banner (ingen tracking-cookies finnes).
   - ~~Search Console-innsending og indekseringsverifisering~~ — **Ferdig (Runde 19, 2026-08-14).** Domain property `kvernhaugbrygghus.no` verifisert via DNS TXT hos Domeneshop (én permanent TXT-record, ingen andre DNS-endringer). `sitemap.xml` sendt inn og akseptert — Search Console rapporterer status «Fullført», 18/18 URL-er oppdaget (matcher 9 NO + 9 EN). Manuell «Be om indeksering» sendt for de 4 viktigste sidene (`/`, `/en/`, `/hjelp/`, `/en/hjelp/`); resten går via sitemap. Første URL Inspection viste «Oppdaget – ikke indeksert for øyeblikket», uten teknisk blokkering — forventet status for et nettsted publisert samme dag. Videre indekseringsstatus avhenger nå av Googles egen crawl-takt, ikke av noe i koden.
   - **Gjenstår, ikke en blocker:** `www.kvernhaugbrygghus.no` serverer i dag identisk innhold direkte i stedet for å 301-redirecte til non-www — `canonical`-taggen peker uansett korrekt til non-www, så dette er en lav-prioritets hosting-opprydning (SHOULD), ikke en lanseringsblocker. Open Graph/Twitter Cards, structured data (JSON-LD), analytics/tracking — bevisst fortsatt utenfor omfang.
