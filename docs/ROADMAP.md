...
7. **Web — fremtidig SEO-URL-strategi**: Runde 14 valgte klientside språkbytte på samme URL for V1 (se `web/README.md` "Språk (NO/EN)"). Skal engelsk innhold (inkl. den nå ferdigoversatte hjelp/bryggehåndboken fra Runde 14B) være reelt søkbart, trengs et avgrenset pre-render-steg ved deploy (statiske `/en/*.html`-snapshots generert fra samme `TEKSTER`-ordbok) — egen runde, ikke påbegynt. Del av punkt 8 sitt SEO-grunnarbeid.
8. **Web-versjon — Pre-deploy / lanseringsklar-runde** (før publisering på `KvernhaugBrygghus.no`, ikke påbegynt): vurdere endelig funksjonsomfang for første offentlige versjon, innholdskvalitet i Hjelp/bryggehåndbok, datakvalitet for malt/humle/gjær, stilbibliotek/dekning, visuell sluttpolish på mobil/desktop, utskriftskontroll med ekte oppskrifter, hosting/opplasting, kobling til `KvernhaugBrygghus.no`, og en produksjonstest etter deploy. Kjente delkrav som ikke må forsvinne før lansering:
   - **Norsk + engelsk er must-have** — app-UI og hjelp/bryggehåndbok begge ferdig oversatt (Runde 14 + 14B); egen crawlbar engelsk URL gjenstår (punkt 7); Bryggmester-innholdshullene i punkt 6 bør også vurderes før lansering.
