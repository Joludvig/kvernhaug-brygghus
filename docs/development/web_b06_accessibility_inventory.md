...
| 13 | Brew-log tasting-judgment button group (yes/maybe/no) | `web/bryggelogg.html:133-137` | `role="group"` with no `aria-label`/`aria-labelledby` — the adjacent question text (`.brygg-smak-sporsmal`) is visually obvious but not programmatically tied to the group | SHOULD FIX |
...
**Status: FIXED.** Implemented and shipped in #142 / PR #143 (exact head `cfd4e81e0eded8b339d8d09360636c2dffac5bc4`). These are no longer active MUST FIX findings — the section below is retained as historical evidence of the original defect and how it was fixed, not as a description of the current state.
...
### 7. Brew-log OG / Volume / FG measurement fields — orphaned labels

`web/bryggelogg.html:121-130`:
...
Text is filled in at runtime (`web/js/brygg_page.js:162-163,188`: `kort.querySelector(".brygg-og-label").textContent = t("brygg.ogLabel")`, etc.) but **no `for`/`id` is ever set**, in the template or in JS. The `<label>` and `<input>` are DOM siblings only — visually adjacent, not programmatically linked. Same visible strings ("Målt OG"/"Målt FG") are correctly wired in `web/verktoy.html:86-91` (finding #17) — that is the exact fix pattern (add a stable `id` to each input, add matching `for` to each label, either in the `<template>` markup or via a one-line `label.htmlFor = input.id` when the labels are populated in `brygg_page.js`).

### 8. Brew-log tasting sliders — 18 unlabeled range inputs per brew

`web/js/brygg_page.js:90-113`:
...
Same orphaned-label pattern as #7, text populated the same way in `web/js/brygg_page.js:239,244,246,285` — same fix. One additional detail worth noting: `.brygg-nestegang` does get a `placeholder` fallback (`brygg_page.js:241`), but `.brygg-fungerte`/`.brygg-endret` ("what worked" / "what changed") get **neither** a `for`/`id` link **nor** a placeholder — those two fields currently have zero accessible name and zero visual fallback hint, the weakest case in this group.
...
- **#13** — `web/bryggelogg.html:133-137`: `<div class="modus-bryter brygg-dom-bryter" role="group">` has no `aria-label`/`aria-labelledby`. The preceding `<p class="brygg-smak-sporsmal">` (line 138, populated in `brygg_page.js:214`) supplies the question visually but isn't wired via `aria-labelledby`.
