# KBH Core Contract (V1 — superseded)

Version: 1.0
Status: SUPERSEDED — no longer the governing Core Contract as of
2026-08-31, in its entirety, including §3–§11.
Superseded by: `docs/development/KBH_CORE_CONTRACT.md` (v2.0)

Preserved as historical record. The two-workspace ownership model in
"Background" and §1–§2 below (Streamlit/Web) has been replaced by the
five-domain ownership model in the active contract (Core / App-Web /
Bryggeskole / Brew Lab / Sóti).

This document also documents the **operative legacy `.kbhrecipe`
contract** that today's implementation (`modules/kbh_contract.py`,
`ui/recipe_card.py`) still actually uses (§3–§11: wrapper shape,
payload fields, the whitelist rule, units/normalization, identity
policy, the passthrough law, "no smart guessing", and the relationship
to `.kbhbrew`). That legacy contract is respected for backward
compatibility. It is not, on that account, an alternative active Core
Contract — this entire document is superseded as the governing Core
architecture baseline. Whether the legacy `.kbhrecipe` contract is
re-homed under the Core domain as-is, revised, or replaced is decided
only by a separate, explicitly authorized Core review; no `.kbhrecipe`
change is made as part of this supersession.
