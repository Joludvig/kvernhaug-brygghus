...
- **Type/domain:** a real number of degrees Celsius, or `None`. The UI widget applies only a generous sanity bound (proposed `min_value=0.0, max_value=40.0, step=0.5`) to catch fat-finger input — this is a UI guard against nonsense values, not a brewing-accuracy claim, and is well outside any verified range so it never contradicts §1.1/§1.6's "no universal number" position.
...
- **Session-state ownership:** a single scalar widget key, `st.session_state["gjaering_temp_maal_c"]`, owned directly by the `st.number_input` in `ui/yeast_panel.py` — the same "the widget key itself is what's read/written on save and load" pattern already used for `brygger_stil` (`ui/recipe_card.py:143`) and `batch_volum_input` (`ui/recipe_card.py:140`), **not** the heavier "snapshot key + panel-owned resync/normalize" pattern used for `process_profile`/`water_*` (§1.7d). The lighter pattern is the right fit here because this is one independent scalar with no nested structure and no cross-recipe-switch resync hazard to guard against (unlike a multi-step mash profile) — using the heavier pattern would be unjustified complexity for this shape.
...
An older recipe file saved before this field existed simply lacks the `fermentation_temp_target_c` key. On load, `ui/sidebar.py` reads it with `r_data.get("fermentation_temp_target_c")` (plain `dict.get`, no exception, no branch needed for "key missing" vs. "key present but null" — both already yield Python `None`) and sets `st.session_state["gjaering_temp_maal_c"] = None` in that case, exactly mirroring the `_aktiv_recipe_efficiency` example in §1.8. The widget then renders empty — **never** a guessed number, and never derived from `gjaertype`/category/flavor tags, satisfying the issue's explicit backward-compatibility requirement. No migration script, no schema version bump, and no repair/normalization step (unlike `process_profile`'s `normaliser_prosessprofil()`) is needed, since a bare scalar has no internal shape that can go stale or corrupt the way a nested mash-step object can.
...
- **Setup:** the evaluator is given a yeast strain already selectable in `ui/yeast_panel.py` (e.g. `saflager_w3470`) together with that strain's actual manufacturer-published fermentation temperature range (supplied by the evaluator from outside the app, e.g. Fermentis's published 9–15°C range for SafLager W-34/70 — not fabricated in-app), deliberately different in framing from `Q-FERM-001`/`Q-FERM-002`'s own numbers/scenario.
...
1. **New or extended `tests/test_yeast_panel.py`** (no such file exists today), using the same `AppTest`-driven pattern as `tests/test_process_panel.py`'s `TestLaerBroMeskingBridge` (`tests/test_process_panel.py:414–474`):
   ...
   - the number input exists, defaults to empty/`None` for a brand-new session, and setting it updates `st.session_state["gjaering_temp_maal_c"]`.
...
3. **A new full-stack save/reload `AppTest` integration test**, following `tests/test_water_recipe_integration.py`'s exact shape (isolated `KVERNHAUG_RECIPES_DIR`, real `app.py` via `AppTest`, save → fresh session → reopen): (a) an old recipe saved *without* the field opens with the widget empty/`None` (backward compatibility, §7); (b) setting a target, saving, and reopening in a brand-new `AppTest` session preserves the exact value; (c) malt/hops/yeast/`process_profile`/`water_*` are unchanged by this feature (mirrors that test file's own "ingredients/process_profile unchanged" assertions).
...
2. **Exact wording of the guardrail caption (§8)** — the constraint (must not imply a universal or app-known-correct range) is fixed here; the literal NO/EN sentence is left to the implementation child, consistent with this document not pre-baking pilot/UI copy any more precisely than the Mesking precedent did for its own bridge footer.

No other genuine product choice in this slice required an owner tie-break beyond Chief's review of this contract itself — every question in issue #382's "Required repo audit" and "UX decision required" sections was resolvable from existing, already-established repo precedent (the verified-only Course Fact Registry boundary, the already-shipped Mesking Learn→Plan bridge mechanics, the existing `water_*`/`process_profile` optional-field persistence pattern, and the explicit `.kbhrecipe` whitelist rule in `CORE_KBHRECIPE_V1.md`).
