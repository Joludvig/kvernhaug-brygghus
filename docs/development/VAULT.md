...
En separat, frittstående Obsidian-vault ved `D:\Vault\Kvernhaug Brygghus` — **ikke** en del av dette app-repoet, og ikke git-sporet sammen med det. Den inneholder en "Canon Foundation": kanoniske notater om ølidentiteter, kildehistorikk og prosjektstatus, bygget med en status/source_layer-metadatamodell (FACT/PROBABLE-merking av kildemateriale under `Canon/Kilder/`) og et "Handover"-dokument som bevisst bygger bro mellom Vault-en og den faktiske kodetilstanden i dette repoet.
