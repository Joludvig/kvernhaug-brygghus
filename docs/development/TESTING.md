...
### Ekte, kjørende JS-dekning av DOM-frie web/js-moduler (WEB PRI 5, issue #51)

De første web-testene i `tests/` (f.eks. `test_web_mode_storage_fix.py`, `test_web_custom_ingredient_id_active_draft.py`) antok at miljøet ikke har noen JavaScript-kjøretid, og verifiserer derfor kun at kildeteksten i `web/js/**` matcher bestemte regex-mønstre ("kilde-kontrakt"-tester) — de kan bevise at koden SER riktig ut, aldri hva den faktisk RETURNERER. Den antakelsen stemmer ikke lenger: CI-runneren har Node.js installert. `tests/web_js_runtime.py` + `tests/js_runtime/eval_web_js.js` laster ordentlige `web/js/*.js`-filer inn i en Node `vm`-kontekst (samme prinsipp som nettleserens `<script>`-tagger, ingen jsdom, ingen npm-avhengighet — se `.claude/rules/web.md`) og evaluerer et ekte JS-uttrykk mot dem, slik at Python-testene kan asserte på faktiske returverdier/golden vectors. Se `tests/test_web_js_calc.py`, `tests/test_web_js_kbhrecipe.py`, `tests/test_web_js_custom_ingredient_id.py` og `tests/test_web_js_brew_storage.py` for eksempler, og `tests/web_js_runtime.py` sin docstring for hvorfor `node` kan kjøres selv om Claudes Bash-verktøy i praksis kun har `python3 -m unittest ...` på sin tillatelsesliste (docs/development/AGENT_WORKFLOW.md) — en tillatt `python3 -m unittest`-prosess kan trygt shelle ut til `node` internt. Foretrekk dette mønsteret for nye DOM-frie web/js-tester fremover; de eldre regex-testene beholdes uendret (de dokumenterer fortsatt reelle, tidligere Chief-review-blokkere) i stedet for å skrives om uten en egen anledning.
