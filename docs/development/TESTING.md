...
Ekte, kjørende browser-/E2E-dekning finnes (Production Workflow V2, #199, P3): `tests/playwright/**` — Playwright-spec-er, en egen kjøremotor atskilt fra Python-`unittest`-suiten over, aldri kjørt via `discover -s tests` — kjører automatisk på hver PR i "Playwright Critical Browser Gate" (`.github/workflows/playwright-browser-gate.yml`): Chromium + Firefox, desktop 1280x900 + mobil 390x844, NO + EN, servert lokalt fra det checked-out PR-hodet (aldri produksjon), med 0 konsoll-/sidefeil for flytene spec-ene dekker. Kjør lokalt med `npm run test:browser-gate` (wrapper rundt `playwright test`, config i `playwright.config.js`). Denne CI-portvakten erstatter ikke manuell verifisering utenfor sitt faste omfang — en bredere/utforskende viewport-sveip eller en fysisk-enhet-sjekk er fortsatt en egen, eier-kjørt runde (se `web-full-regression`-skillet) der den gir bevis porten ikke allerede dekker. Se testpolicy i [`.claude/rules/testing.md`](../../.claude/rules/testing.md) for når full suite faktisk trengs kontra en avgrenset/intermediate runde.
...
En tredje Chief-review (PR #53, hode `3837d3219a9c64dcf9f8f5686299918186ffac8f`) påpekte at tall-/operatorsekvens-sammenligningen over er blind for aritmetiske operatorer, operand-/dataflytstruktur og kall: en Web-only mutasjon som `+=`→`-=`, `*`→`/` eller `1 + (...)`→`1 - (...)` beholder alle tallitteraler og alle sammenligningsoperatorer og ville derfor fortsatt bestått. `TestBeregningsstrukturParitet` i samme fil legger til ekte strukturell (operator-/operandtre) paritet for hver formels kjerneuttrykk: begge språksidenes uttrykkstekst parses, via én liten felles grammatikk (tall, identifikatorer, `+ - * / **`, unær minus, funksjonskall), til et operator-/operandtre, etter normalisering av rent språkspesifikk staving (camelCase↔snake_case, `Math.pow`/`**`, `Math.exp`/`math.exp`, ledende understrek på Python-konstanter) — selve trestrukturen, ikke bare en flat token-sekvens, må matche. `TestMutasjonerFeiler` beviser deretter, for hver av de tre mutasjonstypene reviewen navnga, at strukturtesten faktisk avviser dem (kun en in-memory kopi av calc.js-kildeteksten muteres, aldri selve filen eller annen `web/**`-produktkode).

En fjerde Chief-review (PR #53, hode `98f3caecd5002f2002068498c5b98b8b6b19392a`) påpekte at den erklærte EBC-strukturparitetstesten likevel utelot tre kjerne-forløperuttrykk — `volumGal`, `maltLovibond`, `mengdeLb` (enhetskonverteringene som mater MCU-akkumulatoren) — slik at f.eks. en endring av `const volumGal = volum * LITER_TIL_US_GALLON;` til divisjon beholdt alle tidligere sjekkede trær/sekvenser og fortsatt ville bestått. `_ebc_komponenter()` trekker nå også ut og sammenligner disse tre uttrykkene strukturelt, med en representativ in-memory-mutasjon (`test_multiplikasjon_til_divisjon_i_ebc_volum_gal_feiler`) som beviser at den nye dekningen faktisk avviser en slik endring — verifisert (og reversert igjen) direkte mot den ekte `web/js/calc.js`-filen, ikke bare mot en in-memory-kopi.
